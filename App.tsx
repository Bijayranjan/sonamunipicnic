...
import { MapPin, Users, Wallet, Leaf, Flame, Utensils, Package, ChevronRight, CheckCircle2, Circle, RefreshCcw, Wifi, WifiOff, Calendar, Camera, ArrowUpRight } from 'lucide-react';
                ...
                <div>
                  ...
                  <div className="text-[10px] uppercase font-black tracking-tight">
                    {stats.feesPerMember && member.amountPaid >= stats.feesPerMember 
                      ? <span className="text-emerald-500 font-bold">FULLY PAID</span> 
    ...
    <div className="min-h-screen bg-zinc-50 pb-24 text-zinc-900">
      ...
      <nav className="glass-nav fixed top-0 w-full z-50 flex items-center justify-between px-6 py-4">
        ...
        <a 
          ...
          className="bg-zinc-900 text-white p-2 rounded-full hover:bg-black transition-all active:scale-90 shadow-lg shadow-zinc-200 flex items-center justify-center"
        >
          <MapPin size={20} />
        </a>
      </nav>
      ...
      <div className="pt-20">
        ...
        <div className="px-6 mb-8">
          <div className="cred-card p-6 rounded-[2.5rem] spot-gradient overflow-hidden relative border-none shadow-xl shadow-zinc-200/50">
            <div className="flex justify-between items-start mb-6">
              <div className="flex items-center gap-2">
                 <div className="w-10 h-10 rounded-2xl bg-white shadow-sm flex items-center justify-center border border-zinc-50">
                   <MapPin className="w-5 h-5 text-rose-500" />
                 </div>
                 <div className="flex flex-col">
                    <span className="text-zinc-400 text-[10px] font-black uppercase tracking-widest">Active Spot</span>
                    <span className="text-xs font-bold text-zinc-500">Sonamuni Beach</span>
                 </div>
              </div>
              <a 
                href={PICNIC_LOCATION_URL}
                target="_blank"
                className="w-10 h-10 rounded-full bg-zinc-900 flex items-center justify-center text-white"
              >
                <ArrowUpRight className="w-5 h-5" />
              </a>
            </div>

            <h2 className="text-4xl font-black tracking-tighter text-zinc-900 leading-[0.9] mb-4">
              Jungle <br/> <span className="text-zinc-300">Picnic '25</span>
            </h2>

            <div className="flex flex-wrap items-center gap-2">
              <div className="flex items-center gap-1.5 bg-white/80 backdrop-blur px-3 py-1.5 rounded-full border border-white shadow-sm">
                <Calendar className="w-3.5 h-3.5 text-rose-500" />
                <span className="text-[10px] font-black text-zinc-700 uppercase">28 Dec</span>
              </div>
              <div className="flex items-center gap-1.5 bg-white/80 backdrop-blur px-3 py-1.5 rounded-full border border-white shadow-sm">
                <Camera className="w-3.5 h-3.5 text-blue-500" />
                <span className="text-[10px] font-black text-zinc-700 uppercase">Premium View</span>
              </div>
            </div>
            
            {/* Visual background decoration */}
            <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-zinc-100 rounded-full blur-3xl opacity-50" />
          </div>
        </div>
        ...
        <div className="pb-10 min-h-[300px]">
          {isInitialLoading ? (
             <div className="flex flex-col items-center justify-center py-20 gap-4">
                ...
                <span className="text-zinc-400 text-[10px] font-black uppercase tracking-widest animate-pulse">Syncing Cloud Data</span>
             </div>
          ) : (
            renderTable()
          )}
